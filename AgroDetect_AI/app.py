"""
AgroDetect AI - Flask Web Application
Plant disease detection system using Hugging Face pre-trained model.
Model: nateraw/plant-disease (ResNet50 fine-tuned on New Plant Diseases Dataset)
"""
...
MODEL_NAME = 'nateraw/plant-disease'
...
def load_model():
    """
    Load the Hugging Face pre-trained model and feature extractor.
    Uses global caching to avoid reloading on every request.
...
def predict_disease(image_path):
    """
    Predict plant disease from image file using Hugging Face model.
    
    Args:
        image_path (str): Path to the image file
        
    Returns:
        tuple: (disease_name, confidence_percentage)
    """
    # Load image
    ...
    # Get disease name from model config
...
def predict():
    ...
    Process the uploaded image, run disease classification, and display results.
    """
    try:
...
if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("AGRODETECT AI - STARTING WEB APPLICATION")
    print("=" * 60)
    print(f"Upload folder: {UPLOAD_FOLDER}")
    print(f"Allowed extensions: {ALLOWED_EXTENSIONS}")
    print(f"Model: {MODEL_NAME} (Hugging Face)")
    print("=" * 60 + "\n")
